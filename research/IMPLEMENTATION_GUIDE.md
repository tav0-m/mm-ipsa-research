...
```powershell
.\.venv\Scripts\mm-ipsa.exe run --step download
...
```powershell
.\.venv\Scripts\mm-ipsa.exe run --step transform
...
**Implementación:** Gaussiano terminal, Student-t terminal e histórico EWMA reciben los mismos targets H.

```powershell
.\.venv\Scripts\mm-ipsa.exe run --step benchmarks
...
```powershell
.\.venv\Scripts\mm-ipsa.exe run --step evaluate
...
**Implementación:** `src/mm_ipsa/evaluation/scoring.py` guarda CRPS, Energy y Variogram por cada una de las 121 ventanas. Energy estima una sola vez por modelo su término entre escenarios con 100.000 pares, evitando un sorteo diferente por fecha. `src/mm_ipsa/evaluation/comparison.py` calcula `MM − benchmark`, remuestrea bloques móviles de cuatro ventanas durante 5.000 iteraciones, construye un IC95 básico con la distribución centrada bajo H0 y aplica Holm a la familia de nueve contrastes.
...
```powershell
.\.venv\Scripts\mm-ipsa.exe run --step portfolio
...
```powershell
.\.venv\Scripts\mm-ipsa.exe run --step backtest
...
**Implementación:** `research/liquidity_robustness.yaml` congela una regla que usa únicamente `zero_return_rate_is <= 3%` y `max_zero_run_is <= 5`. `src/mm_ipsa/analysis/liquidity_robustness.py` selecciona el universo, recalibra MM y los tres benchmarks con idénticos hiperparámetros, vuelve a optimizar portafolios y repite scores/backtest. OOS se adjunta solo como diagnóstico ex post; no participa en `selected`.

```powershell
.\.venv\Scripts\python.exe -m unittest tests.test_liquidity_robustness -v
.\.venv\Scripts\mm-ipsa.exe run --step liquidity
...
**Implementación:** suite `unittest`, `requirements-lock.txt`, paquete instalable, manifest con SHA-256/versiones y comando de verificación con exit code no cero.
