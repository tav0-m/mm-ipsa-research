...
from src.analysis.liquidity_robustness import load_liquidity_config, select_liquid_universe
from src.analysis.rolling_origin import load_rolling_origin_config
from src.mm.objective import MMObjective
from src.mm.targets import compute_targets
from src.lineage import validate_lineage
...
def main() -> int:
    ...
    if all(path.exists() for path in mm_paths):
        ...
        verifier.check(np.all(p >= 0) and np.isclose(p.sum(), 1.0), "simplex de probabilidades")
    ...
    if args.scope == "full":
        ...
        for universe, summary_path, observation_path, difference_path in score_paths:
            ...
            verifier.check(
                differences[["pvalue_raw", "pvalue_holm"]].apply(
        ...
        if all(path.exists() for path in rolling_paths):
            ...
            observed_temporal_order = all(
                pd.Timestamp(row.train_end_actual)
                < pd.Timestamp(row.evaluation_start_actual)
                <= pd.Timestamp(row.evaluation_end_actual)
