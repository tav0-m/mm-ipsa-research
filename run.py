...
from src.lineage import assert_lineage_current, write_lineage
...
def _common_lineage_inputs() -> list[Path]:
    return [ROOT / "run.py", ROOT / "config.yaml", ROOT / "src" / "config.py", ROOT / "src" / "lineage.py"]
...
def _read_returns(path: Path, labels: list[str]) -> pd.DataFrame:
    ...
    result = frame[labels].sort_index()
...
def _compute_targets(cfg: dict) -> tuple[np.ndarray, np.ndarray, np.ndarray, pd.DataFrame]:
    from src.mm.targets import compute_targets, save_targets
...
def step_transform(cfg: dict):
    ...
    _ensure_dirs(cfg)
    from src.data.transform import build_returns

    price_path = Path(cfg["paths"]["data_raw"]) / "adj_close_prices.csv"
    if not price_path.exists():
        raise FileNotFoundError("Ejecute primero: python run.py --step download")
    download_manifest = _lineage_path(cfg, "download")
    upstream: list[Path] = []
    source_status = "legacy_canonical_without_download_manifest"
    if download_manifest.exists():
        upstream.append(_require_stage(cfg, "download"))
...
def step_mm(cfg: dict):
    ...
    from src.mm.diagnostics import MMDiagnostics
    ...
    _write_stage(
        ...
        "mm",
        [
            ROOT / "src" / "mm" / "objective.py",
            ROOT / "src" / "mm" / "bcd.py",
            ROOT / "src" / "mm" / "targets.py",
            transform_manifest,
            out / "daily_returns.csv",
            out / f"hist_terminal_returns_H{H}.csv",
        ],
...
def step_benchmarks(cfg: dict):
    ...
    _ensure_dirs(cfg)
    from src.mm.targets import _ewma_weights
...
def step_evaluate(cfg: dict):
    ...
    _write_stage(
        ...
        "evaluation",
        [
            ROOT / "src" / "evaluation" / "scoring.py",
            ROOT / "src" / "evaluation" / "comparison.py",
            mm_manifest,
            benchmark_manifest,
            oos_path,
            *model_inputs,
...
def step_portfolio(cfg: dict):
    ...
    _write_stage(
        ...
        "portfolio",
        [
            ROOT / "src" / "portfolio" / "optimization.py",
            mm_manifest,
            benchmark_manifest,
            out / "targets_cov.csv",
            *model_inputs,
...
def step_backtest(cfg: dict):
    ...
    from src.portfolio.optimization import equal_weight, hierarchical_risk_parity, inverse_variance
    ...
    static_weights = {
        name: group.set_index("asset").loc[labels, "weight"].to_numpy()
    ...
    _write_stage(
        ...
        "backtest",
        [
            ROOT / "src" / "backtest" / "walk_forward.py",
            ROOT / "src" / "portfolio" / "optimization.py",
            transform_manifest,
            portfolio_manifest,
            out / "daily_returns.csv",
...
def step_liquidity_robustness(cfg: dict):
    ...
    _ensure_dirs(cfg)
    from src.analysis.liquidity_robustness import (
    ...
    _write_stage(
        ...
        [
            ROOT / "src" / "analysis" / "liquidity_robustness.py",
            ROOT / "src" / "mm" / "objective.py",
            ROOT / "src" / "mm" / "bcd.py",
            ROOT / "src" / "mm" / "targets.py",
            ROOT / "src" / "models" / "benchmarks.py",
...
def step_rolling_origin(cfg: dict):
    ...
    _write_stage(
        ...
        "rolling_origin",
        [
            ROOT / "src" / "analysis" / "rolling_origin.py",
            ROOT / "src" / "data" / "transform.py",
            ROOT / "src" / "mm" / "objective.py",
            ROOT / "src" / "mm" / "bcd.py",
            ROOT / "src" / "mm" / "targets.py",
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        required=True,
        choices=[
            "download", "transform", "mm", "benchmarks", "wiener", "evaluate",
            "reuse-download", "compare", "portfolio", "backtest", "liquidity", "rolling", "snapshot", "all",
        ],
    )
    ...
    steps = {
        ...
        "wiener": step_benchmarks,
        "evaluate": step_evaluate,
        "compare": step_evaluate,
    ...
    if args.step == "all":
        for step in (
            "download", "transform", "mm", "benchmarks", "evaluate", "portfolio",
            "backtest", "liquidity", "rolling", "snapshot",
        ):
            steps[step](cfg)
    else:
        steps[args.step](cfg)
